...
int main() {
  ...
  unsigned engine_runs_dst_V = 64;

  // 3. Custom registers
  unsigned width_val = 64;
  unsigned height_val = 64;
  ...
  for(i=0; i<dst_V_height; i++){
    for(j=0; j<dst_V_width; j++){
      int32_t dut_val = dst_V_l1[i*dst_V_width+j];
      int32_t ref_val = dst_V_golden_l1[i*dst_V_width+j];
